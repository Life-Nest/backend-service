...
export async function getUsers(req, res) {
  try{
    ...
    res.status(500).json({
      msg: 'server error getting all the users failed',
      error: err.message
    });
  }
}
...
export async function getUser(req, res) {
  try {
    ...
    res.status(500).json({
      msg: 'server Error getting user failed',
      error: 'err.message'
    });
  }
}
...
export async function updateUser(req, res) {
  try {
    ...
    res.status(500).json({
      msg: "server Error updating user failed",
      error: err.message
    });
...
export async function deleteUser(req, res) {
  ...
  try {
    ...
    res.status(500).json({
      msg: 'server Error deleting user failed',
      error: err.message
    });
  }
}